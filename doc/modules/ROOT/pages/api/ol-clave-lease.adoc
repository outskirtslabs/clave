...
Call <<ensure-active,`ensure-active`>> at the top of your function and at natural checkpoints: the
start of each loop iteration, before issuing a network request, or after
returning from a potentially slow sub-call.
If the lease has been cancelled, <<ensure-active,`ensure-active`>> throws an exception containing
...
See also: <<with-cancel,`with-cancel`>>, <<with-timeout,`with-timeout`>>, <<with-deadline,`with-deadline`>>, <<ensure-active,`ensure-active`>>
