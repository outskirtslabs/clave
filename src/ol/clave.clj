(ns ol.clave
  "Porcelain API for ACME certificate acquisition.
  ...
  The main entry point is [[obtain-certificate]] which orchestrates the
  complete ACME workflow: creating orders, solving challenges, finalizing
  with a CSR, and downloading the issued certificate.
...
(defn identifiers-from-sans
  ...
  ```"
  [sans]
  (let [parse-ip (requiring-resolve 'ol.clave.impl.parse-ip/from-string)]
    (mapv (fn [san]
            (if (parse-ip san)
...
(defn obtain-certificate-for-sans
  ...
  (obtain-certificate the-lease session (identifiers-from-sans sans) cert-key solvers {}))
