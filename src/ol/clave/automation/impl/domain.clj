...
(defn validate-domain
  ...
  (cond
    ...
    (= domain "localhost")
    (invalid-domain domain "localhost is not a valid ACME domain - ACME certificates require publicly resolvable domain names")

    (some #(str/ends-with? domain %) invalid-tlds)
    (invalid-domain domain (str domain " uses a reserved TLD that cannot receive ACME certificates - .local, .internal, and .test domains are not publicly resolvable"))
